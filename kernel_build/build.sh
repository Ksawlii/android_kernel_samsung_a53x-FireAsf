...
make -j$(nproc --all) KCFLAGS="${KCFLAGS} -gdwarf-4" -C $(pwd) O=out $BUILD_ARGS a53x_defconfig >/dev/null
make -j$(nproc --all) KCFLAGS="${KCFLAGS} -gdwarf-4" -C $(pwd) O=out $BUILD_ARGS dtbs >/dev/null
make -j$(nproc --all) KCFLAGS="${KCFLAGS} -gdwarf-4" -C $(pwd) O=out $BUILD_ARGS >/dev/null
make -j$(nproc --all) KCFLAGS="${KCFLAGS} -gdwarf-4" -C $(pwd) O=out INSTALL_MOD_STRIP="--strip-debug --keep-section=.ARM.attributes" INSTALL_MOD_PATH="$MODULES_OUTDIR" modules_install >/dev/null
