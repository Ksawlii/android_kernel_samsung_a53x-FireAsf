...
OUT_KERNELZIP="$(pwd)/kernel_build/FireAsf-${FIRE_VERSION}-KSU-Testing231_a53x.zip"
OUT_KERNELTAR="$(pwd)/kernel_build/FireAsf-${FIRE_VERSION}-KSU-Testing231_a53x.tar"
...
# Kernel-side
BUILD_ARGS="LOCALVERSION=-FireAsf-${FIRE_VERSION}-KSU-TestingAsf KBUILD_BUILD_USER=Ksawlii KBUILD_BUILD_HOST=FireAsFuck"
...
make -j$(nproc --all) -C KCFLAGS="${KCFLAGS} -gdwarf-4" $(pwd) O=out $BUILD_ARGS a53x-ksu_defconfig >/dev/null
make -j$(nproc --all) -C KCFLAGS="${KCFLAGS} -gdwarf-4" $(pwd) O=out $BUILD_ARGS dtbs >/dev/null
make -j$(nproc --all) -C KCFLAGS="${KCFLAGS} -gdwarf-4" $(pwd) O=out $BUILD_ARGS >/dev/null
make -j$(nproc --all) -C KCFLAGS="${KCFLAGS} -gdwarf-4" $(pwd) O=out INSTALL_MOD_STRIP="--strip-debug --keep-section=.ARM.attributes" INSTALL_MOD_PATH="$MODULES_OUTDIR" modules_install >/dev/null
